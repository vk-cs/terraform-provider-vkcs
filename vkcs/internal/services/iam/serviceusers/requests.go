...
func Create(client *gophercloud.ServiceClient, projectID string, opts CreateOptsBuilder) (r CreateResult) {
	...
	resp, err := client.Post(serviceUsersURL(client, projectID), &b, &r.Body, &gophercloud.RequestOpts{
		OkCodes: []int{200, 201},
	})
...
func Get(client *gophercloud.ServiceClient, projectID, id string) (r GetResult) {
	resp, err := client.Get(serviceUserURL(client, projectID, id), &r.Body, &gophercloud.RequestOpts{
		OkCodes: []int{200},
	})
	_, r.Header, r.Err = gophercloud.ParseResponse(resp, err)
	return
...
func List(client *gophercloud.ServiceClient, projectID string, opts ListOptsBuilder) (r pagination.Pager) {
	url := serviceUsersURL(client, projectID)
	if opts != nil {
...
func Delete(client *gophercloud.ServiceClient, projectID string, id string) (r DeleteResult) {
	resp, err := client.Delete(serviceUserURL(client, projectID, id), &gophercloud.RequestOpts{
		OkCodes:      []int{200, 204},
		JSONResponse: &r.Body,
	})
	_, r.Header, r.Err = gophercloud.ParseResponse(resp, err)
