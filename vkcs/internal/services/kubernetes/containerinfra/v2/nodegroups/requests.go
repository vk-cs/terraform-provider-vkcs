...
func Update(ctx context.Context, client *gophercloud.ServiceClient, nodeGroupID string, opts UpdateOpts) error {
	reqBody, err := opts.ToNodeGroupUpdateMap()
	if err != nil {
		return err
	}

	tflog.Info(ctx, "UpdateNodeGroupBody", map[string]interface{}{
		"reqBody": fmt.Sprintf("%s", reqBody),
	})
...
func (t Taint) Less(other Taint) bool {
	tCombo := t.Key + t.Value + t.Effect
	otherCombo := other.Key + other.Value + other.Effect
	return tCombo < otherCombo
}
