...
func (d *serviceUserDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	projectID := d.config.GetTenantID()
	...
	if id := data.Id.ValueString(); id != "" {
		...
		var err error
		serviceUser, err = serviceusers.Get(client, projectID, id).Extract()
		if err != nil {
		...
		allPages, err := serviceusers.List(client, projectID, listOpts).AllPages()
		if err != nil {
