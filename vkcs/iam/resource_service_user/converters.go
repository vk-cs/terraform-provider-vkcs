...
func (m *ServiceUserModel) UpdateFromCreateServiceUserResponse(ctx context.Context, createResp *serviceusers.CreateServiceUserResponse) diag.Diagnostics {
	if createResp == nil {
		return nil
	}
	...
	diags.Append(m.UpdateFromServiceUser(ctx, &createResp.ServiceUser)...)
	if diags.HasError() {
		return diags
	}

	m.Login = types.StringValue(createResp.Login)
	m.Password = types.StringValue(createResp.Password)
