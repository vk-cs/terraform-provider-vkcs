...
func (d *kubernetesClusterVersionV2DataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating Managed K8S API client for datasource vkcs_kubernetes_versions_v2", err.Error())
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Error calling Managed K8S API to get Kubernetes versions", err.Error())
		return
	}
