...
// InsecureRegistriesValidator validates insecure registry URL format
type InsecureRegistriesValidator struct{}

func (v InsecureRegistriesValidator) Description(ctx context.Context) string {
	return "registry URL must be in format: hostname[:port][/path]"
}

func (v InsecureRegistriesValidator) MarkdownDescription(ctx context.Context) string {
	return "Registry URL must be in format: `hostname[:port][/path]`"
}

func (v InsecureRegistriesValidator) ValidateString(ctx context.Context, req validator.StringRequest, resp *validator.StringResponse) {
