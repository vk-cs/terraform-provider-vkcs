...
const (
	ngV2defaultTimeout = time.Minute * 30
)
...
func (r *kubernetesNodeGroupV2Resource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating cluster's node group", err.Error())
		return
	}
	...
	createTimeout, diags := data.Timeouts.Create(ctx, ngV2defaultTimeout)
	if diags.HasError() {
		resp.Diagnostics.Append(diags...)
		return
	}
	...
	stateConf := r.getStateConfForNodeGroupCreate(client, createTimeout, data.ClusterId.ValueString())
	_, err = stateConf.WaitForStateContext(ctx)
	if err != nil {
		resp.Diagnostics.AddError("Error waiting for cluster's node group to become active", err.Error())
		return
	}
...
func (r *kubernetesNodeGroupV2Resource) getStateConfForNodeGroupCreate(client *gophercloud.ServiceClient, createTimeout time.Duration, clusterID string) *retry.StateChangeConf {
...
func (r *kubernetesNodeGroupV2Resource) readNodeGroup(client *gophercloud.ServiceClient, nodeGroupID string) (nodeGroup *nodegroups.NodeGroup, diags diag.Diagnostics) {
	...
	if err != nil {
		...
		diags.AddError("Error reading cluster's node group", err.Error())
		return
	}
...
func (r *kubernetesNodeGroupV2Resource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error updating cluster's node group", err.Error())
		return
	}

	// Parse timeout for operation
	updateTimeout, diags := plan.Timeouts.Update(ctx, ngV2defaultTimeout)
	if diags.HasError() {
		resp.Diagnostics.Append(diags...)
		return
	}

	stateConf := r.getStateConfForNodeGroupUpdate(client, updateTimeout, plan.ClusterId.ValueString())
	_, err = stateConf.WaitForStateContext(ctx)
	if err != nil {
		resp.Diagnostics.AddError("Error waiting for cluster's node group to become active", err.Error())
		return
...
func (r *kubernetesNodeGroupV2Resource) getStateConfForNodeGroupUpdate(client *gophercloud.ServiceClient, updateTimeout time.Duration, clusterID string) *retry.StateChangeConf {
	return &retry.StateChangeConf{
		Pending: []string{
			clusterStatusV2Reconciling,
		},
...
func (r *kubernetesNodeGroupV2Resource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error deleting cluster's node group", err.Error())
		return
	}

	// Parse timeout for operation
	deleteTimeout, diags := data.Timeouts.Delete(ctx, ngV2defaultTimeout)
	if diags.HasError() {
		resp.Diagnostics.Append(diags...)
		return
	}

	stateConf := r.getStateConfForNodeGroupDelete(client, deleteTimeout, data.Id.ValueString())
	_, err = stateConf.WaitForStateContext(ctx)
	if err != nil {
		resp.Diagnostics.AddError("Error waiting for cluster's node group to be deleted", err.Error())
		return
	}
}

func (r *kubernetesNodeGroupV2Resource) getStateConfForNodeGroupDelete(client *gophercloud.ServiceClient, deleteTimeout time.Duration, nodeGroupID string) *retry.StateChangeConf {
	return &retry.StateChangeConf{
		Pending: []string{
