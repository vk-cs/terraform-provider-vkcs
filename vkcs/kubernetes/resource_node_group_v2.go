...
const (
	...
	createNgPollIntervalV2 = 30 * time.Second

	deleteNgDelayV2        = 2 * time.Minute
	deleteNgPollIntervalV2 = 30 * time.Second

	updateNgDelayV2        = 3 * time.Minute
	updateNgPollIntervalV2 = 30 * time.Second
)
...
func (r *kubernetesNodeGroupV2Resource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating Managed K8S API client for resource vkcs_kubernetes_node_group_v2", err.Error())
		return
	}
