...
func expandKeyManagerACL(v interface{}, aclType string) acls.SetOpt {
	users := []string{}
...
func expandKeyManagerACLs(v interface{}) acls.SetOpts {
	var res []acls.SetOpt
		...
		for _, v := range v {
			if v, ok := v.(map[string]interface{}); ok {
				for aclType, v := range v {
					acl := expandKeyManagerACL(v, aclType)
					res = append(res, acl)
				}
			}
		}
...
func flattenKeyManagerACLs(acl *acls.ACL) []map[string][]map[string]interface{} {
