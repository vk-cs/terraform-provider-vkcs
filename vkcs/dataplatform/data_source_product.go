...
func (d *productDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	products = filterProductsByName(productsResp.Products, productName)

	productVersion := data.ProductVersion.ValueString()
	if productVersion != "" {
		products = filterProductsByVersion(products, productVersion)
	}
...
func filterProductsByName(products []products.Product, productName string) (filtered []products.Product) {
...
func filterProductsByVersion(products []products.Product, productVersion string) (filtered []products.Product) {
	for _, product := range products {
